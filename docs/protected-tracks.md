...
Не скачивается: отрывки (`SNIP`), треки по подписке (`SUB`, `GO_PLUS`) и **треки под DRM** — о них
ниже, это главный предел.
...
защиты внутри плеера, а не HTTPS-ссылка. Достать его можно только взломом защиты контента — Arsound
этого не делает.
...
- «Удалить скачанный файл» — в меню трека, появляется только у треков, которые скачал Arsound.
- «Удалить скачанные треки» — в меню плейлиста, с подтверждением и числом файлов.

**Нюанс.** Файл удаляется `File.delete()`, а если не вышло — через `MediaStore` по имени и пути
`%Music/Arsound%`: после переустановки Arsound файлы принадлежат не ему, и прямое удаление
запрещено. Если файла нет вовсе, удаление считается успешным: трек всё равно перестаёт числиться
скачанным.

Импортированную с телефона музыку это не трогает ни в одном из двух случаев: она не проходит через
загрузчик и не числится в списке скачанных. Её пользователь удаляет вручную, по одному треку.
